namespace ToteBetting.BL.Products
{
    ...
    /// <summary>
    /// Calculator for QUINELLA Product
    /// </summary>
    internal class QuinellaCalculator : IDividendCalculator
    {
        /// <inheritdoc />
        ...
        public IDictionary<int, double> DoCalculate(IList<BetDataStruct> betsList, double commissionPercent)
        {
            ...
            foreach (var bet in betsList)
            {
                ...
                if ((bet.Runners[0] == result.RunnersList[0] || bet.Runners[0] == result.RunnersList[1])
                    && (bet.Runners[1] == result.RunnersList[0] || bet.Runners[1] == result.RunnersList[1]))
                {
